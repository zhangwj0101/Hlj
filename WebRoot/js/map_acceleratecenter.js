...
function generateMap(url,div_id,size,level) {
	$.ajax({
		type : "post",
		async : true, // 是否异步加载数据
		url : url,
...
//		console.log(collegeList);
		for (var i = 0, length = collegeList.length; i < length; i++) {
			...
			if (collegeList[i].level == "省级"){
				marker.setIcon(icon_blue);
			}else if (collegeList[i].level == "国家级"){
	...
	function openInfoWindow(marker, college) {
		marker.addEventListener("click", function(e) {
			// 弹出信息窗口
			var content = "<strong><font size='5px'><a href='javascript:searchshow(\""+college.link+"\")'>" + college.name + "</a></strong>";
